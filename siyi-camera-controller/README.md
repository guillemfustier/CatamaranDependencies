...
## Camera Teleoperation node
A teleoperation node is included in the package, which allows controlling the camera gimbal and zoom with keyboard inputs. To run the teleoperation node, in a separate terminal, run:
```bash
