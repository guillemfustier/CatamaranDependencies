...
class SIYISDK:
    ...
    def parseAttitudeMsg(self, msg:str, seq:int):
        try:
            ...
            # print(f"(yaw, pitch, roll= ({self._att_msg.yaw}, {self._att_msg.pitch}, {self._att_msg.roll})")
            # print(f"(yaw_speed, pitch_speed, roll_speed= ({self._att_msg.yaw_speed}, {self._att_msg.pitch_speed}, {self._att_msg.roll_speed})")
            return True
        except Exception as e:
            self._logger.error("Error %s", e)
            return False
    ...
    def setGimbalRotation(self, yaw, pitch, err_thresh=1.0, kp=4):
        """
        Sets gimbal attitude angles yaw and pitch in degrees, con control robusto de error angular.
        ...
        def angular_error(target, current):
            # Calcula el error angular mínimo, resultado en [-180, 180]
            error = (target - current + 180) % 360 - 180
            return error

        # Remapea el feedback de pitch a un rango continuo de 44.9 (arriba) a -134.2 (abajo)
        def remap_pitch(current):
            if 179.4 <= current <= 180.5:
                return 0.0
            elif -180.5 <= current <= -179.4:
                return 0.0
            elif -179.9 < current < -134:
                return current + 179.9
            elif 44.5 < current < 179.9:
                return current - 179.9
            else:
                return current  # fuera de rango, no remapea

        if (yaw > 317 or yaw < -355):
            self._logger.error(f"Desired yaw {yaw} is outside controllable range -355~317")
            return
        ...
        while n <= 100:
            n += 1
            print("==ENTRO LOOP==")
            print("Desired yaw: ", yaw, " pitch: ", pitch)
            self.requestGimbalAttitude()
            # print("Attitude msg seq: ", self._att_msg.seq, " last seq: ", self._last_att_seq)
            if self._att_msg.seq == self._last_att_seq:
                print("Did not get new attitude msg")
                self.requestGimbalSpeed(0, 0)
                continue

            self._last_att_seq = self._att_msg.seq

            yaw_err = angular_error(yaw, self._att_msg.yaw)
            pitch_err = pitch - (-remap_pitch(self._att_msg.pitch))
            
            # PRINTS FOR DEBUGGING
            # print("Desired pitch: ", pitch)
            # print("Current pitch: ", self._att_msg.pitch)
            # print("Remapped current pitch: ", remap_pitch(self._att_msg.pitch))
            # print(f"yaw error: {yaw_err}, pitch error: {pitch_err}, threshold: {th}")

            # Check if the errors are within the threshold
            if (abs(yaw_err) <= th and abs(pitch_err) <= th):
                self.requestGimbalSpeed(0, 0)
                # print("Goal rotation is reached")
                break
